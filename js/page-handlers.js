...
$(document).ready(function() {
  $('body')
    ...
    .delegate('.tile-card', 'click', function() {
      loadDiv('Explore-' + $(this).data('id'));
      return false;
    })
    .delegate('.my-app-card', 'click', function() {
      loadDiv('Explore-' + $(this).data('id'));
      return false;
    })
   ...
   // XXX fix scrolling alert(getCurrentSection());
...
function filterCheckboxClick(element) {
  ...
  if (checked.length === 0) {
    ...
    $('#AppGallery ' + container + ' .intent input:checked').each(function(i, elem) {
      intent.push($(elem).attr('name'));
    });

    $('#AppGallery ' + container + ' .stat input:checked').each(function(i, elem) {
      stat.push($(elem).attr('name'));
    });
    ...
    if(stat.length > 0)
      params.push("stat=" + stat.join(','));

    if(intent.length > 0)
      params.push("intent=" + intent.join(','));
...
function loadDiv(app) {
  ...
  // XXX restore nav feedbac at some point
  //$('.iframeLink,.your-apps,header div.nav a').removeClass('blue');
  $("#Filter .selected-section").removeClass('selected-section');
...
function splitApp(app) {
  ...
  //alert(app+" "+topSection+" "+subSection+" "+params.author);
...
handlers.AppGallery.Filter = function(params) {
  ...
  if(params.stat)
    filters.stat = params.stat.split(',');
  if(params.intent)
    filters.intent = params.intent.split(',');
 
// XXX pass mine/installed via the function instead
 registry.getByFilter(filters, function(appsObj,stat) {
    var apps = [];
    var breadcrumbs = [];

    console.log('stat '+stat);

    for(var i in appsObj) 
    {
	  apps.push(appsObj[i]);
	}
    for(var j in filters.stat) breadcrumbs.push({stat:filters.stat[j], name:prettyName(filters.stat[j])});
    for(var k in filters.intent) breadcrumbs.push({intent:filters.intent[k], name:prettyName(filters.intent[k])});
    for(var l in filters.types) breadcrumbs.push({type:filters.types[l], name:prettyName(filters.types[l])});
    for(var m in filters.services) breadcrumbs.push({service:filters.services[m], name:prettyName(filters.services[m])});
    ...
    generateBreadCrumbs({filter:true, filters:breadcrumbs}, function(breadcrumbHTML) {
      ...
      generateAppFilters(function(filterHTML) {
        ...
        for(var k in filters.types) {
          $('#AppGallery #Filter input[name=' + filters.types[k] +']').prop('checked', true);
        }

        for(var l in filters.services) {
          $('#AppGallery #Filter input[name=' + filters.services[l] + ']').prop('checked', true);
        }
        
        console.log("gallery "+gallery);

        if (gallery || stat=='mine') $('#AppGallery #Filter').html(breadcrumbHTML);
        $('#AppGallery #Filter').append(filterHTML);

	    //console.log('generateAppsHtml');
	
        if (stat=='installed') 
	      generateTilesHtml(apps, function(html) {
	      //console.log('generateAppsHtml returns '+html);
          if (typeof html == 'undefined' || !html) {
            $('#AppGallery #Filter').append("<div id='no-results' style='font-size:26px;padding:5px;'>No app like that exists... yet. Why don't you <a href='develop#Develop-BuildAnApp' class='orange'>create the first</a>?</div>");
          } else {
            $('#AppGallery #Filter').append(html);
          }
        });
        else if (stat=='mine') 
	      generateMyAppsHtml(apps, function(html) {
	      //console.log('generateAppsHtml returns '+html);
		  sync = '<a href="#" class="sync-button action-button" data-provider="github">Sync GitHub</a>';
          if (typeof html == 'undefined' || !html) {
            $('#AppGallery #Filter').append("<div id='no-results' style='font-size:26px;padding:5px;'>No app like that exists... yet. Why don't you <a href='develop#Develop-BuildAnApp' class='orange'>create the first</a>? Or "+sync+"</div>");
          } else {
		    //$('.').append(sync);
            $('#AppGallery #Filter').append(html);
          }
  ...
  registry.getApp(params.app, function(app) {
    generateBreadCrumbs({app:app},function(appHTML) {
      ...
      generateAppDetailsHtml(app, function(html) {
       ...
       // if (!app.name)
       //   app.name="No Name";
        if (app) doAppHeader(app.name, '#appDiv .app-header');
      });
    });
  });
};
